...
def calculate_statistics(x, column_name):
    ...
    for i in x:
        ...
        try:
            ...
            if y == float('nan') or y == float('+inf') or y == float('-inf'):
                if 'string' in hs:
...
#total_time = 0
spark = SparkSession.builder.appName(str(file_name)).getOrCreate()
#for i in ll
try:
    dataset = spark.read.options(header='true', inferschema='false', sep='\t').csv(file_name)
    ...
    with open("testjsons/" + file_name.split("/")[-1].split(".")[0] + ".txt", 'w+') as out_file:
        json.dump(output, out_file)
    out.write(file_name.split("/")[-1] + "\n")
except Exception as e:
    print ("Failed : ", file_name.split("/")[-1])
    print(e)
#gc.collect()
